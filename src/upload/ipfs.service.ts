...
export class IpfsService {
  ...
  async uploadFile(fileBuffer: Buffer, fileName: string): Promise<string> {
    try {
      ...
      const response = await axios.post(`${this.pinningServiceUrl}/pinFileToIPFS`, formData, {
        headers: {
  ...
  async uploadMetadata(metadata: any): Promise<string> {
    try {
      ...
      const response = await axios.post(`${this.pinningServiceUrl}/pinJSONToIPFS`, metadata, {
        headers: {
      ...
      throw new BadRequestException(`IPFS metadata upload failed: ${error.message}`);
    }
  }
